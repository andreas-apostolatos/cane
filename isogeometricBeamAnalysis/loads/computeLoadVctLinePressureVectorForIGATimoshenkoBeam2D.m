...
j1 = findKnotSpan(xib(1),Xi,nu);
j2 = findKnotSpan(xib(2),Xi,nu);
    ...
    if Xi(i+1)-Xi(i)~=0  
        ...
        for j = 1 : length(GP)     
            ...
            for v=1:p+1
                ...
                elseif loadDir == 2
                    F(indexTim1,1) = F(indexTim1,1) + loadFnc*dR(v,1)*n(1,1)*(Xi(i+1)-Xi(i))/2.0*L*GW(j); 
                    F(indexTim2,1) = F(indexTim2,1) + loadFnc*dR(v,1)*n(2,1)*(Xi(i+1)-Xi(i))/2.0*L*GW(j); 
                end
            end
        end
    end
