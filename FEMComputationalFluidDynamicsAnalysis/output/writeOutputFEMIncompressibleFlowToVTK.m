function writeOutputFEMIncompressibleFlowToVTK...
    (analysis,propNLinearAnalysis,propFldDynamics,fldMsh,parameters,up,...
    upDot,upDDot,DOF4Output,caseName,pathToOutput,title,noTimeStep)
%% Licensing
%
% License:         BSD License
...
%          parameters : .nue : The kinematic viscosity
...
[noNodes,~] = size(fldMsh.nodes);

% Number of DOFs
noDOFs = noDOFsNode*noNodes;

% Number of elements in the mesh
[noElements,elementOrder] = size(fldMsh.elements);
...
if isAnalysis3D
	...
	solutionVct = [up(DOF4Output(1,:)), up(DOF4Output(2,:)), zeros(noNodes, 1), up(DOF4Output(3,:))];
end

% Open the file to write out the results
outputUnitColorPlots = fopen(strcat(pathToOutput,caseName,'/',caseName,'_contourPlots_',num2str(noTimeStep),'.vtk'),'w');

% Transpose the nodal coordinates array
XYZ = fldMsh.nodes(1:noNodes,:)';
...
elements(1:elementOrder,1:noElements) = fldMsh.elements(1:noElements,1:elementOrder)' - 1;
...
str = [num2str([ones(noElements, 1)*elementOrder, elements']), repmat(' \n', noElements, 1)]'; 
fprintf(outputUnitColorPlots, reshape(str, 1, size(str, 1) * size(str, 2) ) );
...
str = [num2str(solutionVct(:,4)/parameters.rho), repmat(' \n', noNodes, 1)]'; 
fprintf(outputUnitColorPlots, reshape(str, 1, size(str, 1) * size(str, 2) ) );
...
directoryName = strcat('../../outputVTK/FEMComputationalFluidDynamicsAnalysis/',caseName,'/upRates');
...
outputUnitUPRates = fopen(strcat(directoryName,'/',caseName,'_upRates_',num2str(noTimeStep),'.txt'),'w');
