function [tanMtxLinEl, tanMtxNLinEl, massMtxEl, FBodyEl] = ...
    ...
    (x, y, z, t, upEl, uMeshALEEL, dN, computeBodyForces, parameters, ...
    h, propFldDynamics, isAnalysis3D)
%% Licensing
%
% License:         BSD License
...
%
%           Input :
%         x,y,z,t : Chorochronical location of the Gauss Point in the 
%                   physical space-time interval
...
%
% 9. Compute the tangent matrix for the convection term
%
...
% x-coordinate
dudx = reshape(ptranspose(phorzcat(dN(:, :, 2), ...
    zeros(noElmnts, noDOFsNode, noDOFsNode - 1))), noElmnts, 1, noDOFsEl);
...
% Nonsingleton matrix containing the derivatives of the field wrt 
% y-coordinate
dudy = reshape(ptranspose(phorzcat(dN(:, :, 3), zeros(noElmnts, noDOFsNode, noDOFsNode - 1))), ...
    noElmnts, 1, noDOFsEl);
...
% z-coordinate
if isAnalysis3D
    dudz = reshape(ptranspose(phorzcat(dN(:, :, 4), ...
        zeros(noElmnts, noDOFsNode, noDOFsNode - 1))), noElmnts, 1, noDOFsEl);
...
RPressure = reshape(ptranspose(phorzcat(zeros(noElmnts, noDOFsNode, noDOFsNode-1), ...
    dN(:,:,1))), noElmnts, 1, noDOFsEl);
...
if isAnalysis3D
    BoperatorConvection(:, 1, 1:noDOFsNode:end) = ptranspose(pstimes(dN(:, :, 2),  ...
        convectionVelocity(:, 1, 1)) + pstimes(dN(:,:,3), convectionVelocity(:, 2, 1)) + ...
        pstimes(dN(:, :, 4), convectionVelocity(:, 3, 1)));
else
    BoperatorConvection(:, 1, 1:noDOFsNode:end) = ptranspose(pstimes(dN(:, :, 2), ...
        convectionVelocity(:, 1, 1)) + pstimes(dN(:, :, 3), convectionVelocity(:, 2, 1)));
end
