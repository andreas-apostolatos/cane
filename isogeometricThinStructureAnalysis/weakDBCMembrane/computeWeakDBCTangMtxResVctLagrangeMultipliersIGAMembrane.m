function [tangMtxWeakDBCLM,resVctWeakDBCLM,BSplinePatch] = ...
...
% boundary conditions weakly using the (augmented) Lagrange Multipliers 
% method. The function is intended for weak application of inhomogeneous 
...
%
%                     Output :
%           tangMtxWeakDBCLM : Dummy output array for this function
%            resVctWeakDBCLM : The residual vector corresponding to the
%                              Lagrange Multipliers method for the 
%                              application of weak boundary conditions
...
%
%              1vi.2vi. Get the motion vector for the given time instance
%
...
%
%              1vi.2ix. Compute the element stiffness contribution matrix for the weak application of the Dirichlet boundary conditions and add it to the global vector
%        <-
%   <-
% <-
% 
...
tangMtxWeakDBCLM = 'undefined';
resVctWeakDBCLM = zeros(noDOFs,1);
        ...
        if (weakDBCRgionOnKnotVector(i) ~= weakDBCRgionOnKnotVector(i+1)) || isWeakDBCOverPoint
            ...
            for j = 1:noGPs
                ...
                if BSplinePatch.weakDBC.alpha ~= 0
                    resVctWeakDBCLM(EFT,1) = resVctWeakDBCLM(EFT,1) - ...
                        BSplinePatch.weakDBC.alpha*(RMtx'*g2)*elementLengthOnGP;
                end
                resVctWeakDBCLM(EFTOfCnd(EFTInCnd),1) = resVctWeakDBCLM(EFTOfCnd(EFTInCnd),1) - ...
                    (BLambdaMtx'*g2)*elementLengthOnGP;
            end
        end
    end
end
