function [dHat,CPHistory,resHistory,hasConverged,FComplete,rankD,condK,minEig,...
    BSplinePatches,propCoupling,minElAreaSize] = ...
...
%% 13. Solve the geometrically nonlinear problem
[dHat,CPHistory,resHistory,hasConverged,FComplete,rankD,condK,minEig,...
    BSplinePatches,propCoupling,minElAreaSize] = ...
    solve_IGANLinearSystem...
    (analysis,dHatSaved,dHatDotSaved,dHatDDotSaved,BSplinePatches,connections,...
    dHat,dHatDot,dHatDDot,KConstant,massMtx,dampMtx,...
    computeTanStiffMtxAndResVct,@computeUpdatedGeometryIGAThinStructureMultipatches,...
    freeDOFs,homDOFs,inhomDOFs,valuesInhomDOFs,masterDOFs,slaveDOFs,solve_LinearSystem,...
    t,propCoupling,propTransientAnalysis,propNLinearAnalysis,plot_IGANLinear,...
    isReferenceUpdated,isCosimulationWithEmpire,strcat('\t',tab),graph,outMsg);
...
if strcmp(outMsg,'outputEnabled')
    ...
    fprintf('_______________Linear Analysis Ended_________________\n');
    fprintf('#####################################################\n\n\n');
    fprintf('\n');
end
