function [dHatHistory, resHistory, BSplinePatches, propCoupling, ...
    minElAreaSize] = ...
...
if strcmp(outMsg, 'outputEnabled')
    ...
    if strcmp(couplingMethod, 'Penalty') || ...
            strcmp(couplingMethod, 'augmented Lagrange Multipliers')
        if ~isfield(propCoupling, 'alphaD')
    ...
    elseif strcmp(propCoupling.method, 'lagrangeMultipliers')
        for iConnections = 1:length(connections.lambda)
            ...
            if isfield(connections, 'mu')
                fprintf('Number of elements for mu of connection %d: %d', ...
                    iConnections, length(unique(connections.mu{iConnections}.Xi)) - 1);
            end
        end
        fprintf('\n');
    elseif strcmp(propCoupling.method, 'mortar')
            ...
            if isfield(BSplinePatch, 'weakDBC')
                if isfield(BSplinePatch.weakDBC, 'noCnd')
                    if BSplinePatch.weakDBC.noCnd > 0
                        ...
                        elseif strcmp(BSplinePatch.weakDBC.method, ...
                                'Penalty')
                            if isfield(BSplinePatch.weakDBC.alpha)
...
if (isfield(connections, 'lambda') && strcmp(propCoupling.method, 'lagrangeMultipliers') ) || ...
    ...
    if isfield(connections, 'lambda')
        if isempty(connections.lambda) && strcmp(propCoupling.method, ...
                'lagrangeMultipliers')
            error('Lagrange multipliers discretization for the displacement coupling exists but its empty');
        end
    end
elseif ~isfield(connections, 'lambda') && ...
        strcmp(propCoupling.method, 'lagrangeMultipliers') 
...
for iPatches = 1:noPatches
    ...
    if isWeakDBC(iPatches, 1)
        if strcmp(BSplinePatches{iPatches}.weakDBC.method, ...
                'lagrangeMultipliers')
...
numDOFsPatches = 0;
for iPatches = 1:noPatches
    numDOFsPatches = numDOFsPatches + BSplinePatches{iPatches}.noDOFs;
end
...
%% 8. Compute the total number of DOFs for the coupled multipatch system
if strcmp(propCoupling.method, 'lagrangeMultipliers')
    numDOFs = numDOFsPatches + noDOFsLagrangeMultipliers;
else
    numDOFs = numDOFsPatches ;
end
...
for iPatches = 1:noPatches
    ...
    numCPs_xi = length(BSplinePatches{iPatches}.CP(:, 1, 1));
    
    % Get the number of Control Points in eta-direction
    numCPs_eta = length(BSplinePatches{iPatches}.CP(1, :, 1));
    
    % Initialize the DOF numbering array
    BSplinePatches{iPatches}.DOFNumbering = zeros(numCPs_xi, numCPs_eta, 3);
    ...
    if isWeakDBC(iPatches, 1)
        if strcmp(BSplinePatches{iPatches}.weakDBC.method, 'lagrangeMultipliers')
            for iCnd = 1:BSplinePatches{iPatches}.weakDBC.noCnd
                ...
                numLambda_xi = length(BSplinePatches{iPatches}.weakDBC.lambda{iCnd}.Xi);

                % Initialize the field of the DOF numbering
                BSplinePatches{iPatches}.weakDBC.lambda{iCnd}.DOFNumbering = zeros(numLambda_xi, 3);

                 % Compute the entries of the DOF numbering array
                k = 1;
                for cpi = 1:numLambda_xi
...
if strcmp(propCoupling.method, 'lagrangeMultipliers') || ...
    ...
    for iConnections = 1:connections.No
        ...
        if isLagrangeMultipliersDisplacementsEnabled
            numLambda_xi = length(connections.lambda{iConnections}.CP(:, 1));
        end
        if isLagrangeMultipliersRotationsEnabled
            numMu_xi = length(connections.mu{iConnections}.CP(:, 1));
        end
        ...
        if isLagrangeMultipliersDisplacementsEnabled
            ...
            connections.lambda{iConnections}.DOFNumbering = zeros(numLambda_xi, 3);
        ...
        if isLagrangeMultipliersRotationsEnabled
            % Initialize the field
            connections.mu{iConnections}.DOFNumbering = zeros(numMu_xi, 2);
...
[dHatHistory, resHistory, BSplinePatches, propCoupling, minElAreaSize] = ...
    ...
    valuesInhomDOFs, updateDirichletBCs, masterDOFs, slaveDOFs, nodesALE, ...
