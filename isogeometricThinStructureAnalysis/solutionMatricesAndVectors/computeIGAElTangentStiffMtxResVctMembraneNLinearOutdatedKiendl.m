function [tanMtxEl,resVctEl] = ...
    ...
    (i,p,xi,Xi,j,q,eta,Eta,CP,dR,ACovariant,aCovariant,thickness,Dm,prestress)
%% Licensing
%
% License:         BSD License
...
for rDOFs = 1:noDOFsEl
    ...
    dEpsilonContra(3,1) = 1/2*(dR(kr,2)*aCovariant(dirr,2) + aCovariant(dirr,1)*dR(kr,3));
...
for rDOFs = 1:noDOFsEl
    ...
    for sDOFs = 1:rDOFs
        ...
        if dirr == dirs
            ddEpsilonContra(1) = dR(kr,2)*dR(ks,2);
            ddEpsilonContra(2) = dR(kr,3)*dR(ks,3);
            ddEpsilonContra(3) = 1/2*(dR(kr,2)*dR(ks,3) + dR(kr,3)*dR(ks,2));
        end
