...
% Local number of DOFs (5 DOFs per control point)
numDOFsEl = 5*(p + 1)*(q + 1);

% Number DOFs (5 DOFs per control point)
...
for j = (q + 1):(numKnots_eta - q - 1)
    for i = (p + 1):(numKnots_xi - p - 1)
        if Xi(i + 1) ~= Xi(i) && Eta(j + 1) ~= Eta(j)
            ...
            % Relation global-local dof numbering
            for cpj = j - q:j
                for cpi = i - p:i
                    for dirDOF = 1:5  % 5 DOFs per control point
                        EFT(k) = DOFNumbering(cpi, cpj, dirDOF);
            ...
            for iGP_eta = 1:numGP_eta
                for iGP_xi = 1:numGP_xi
                    ...
                    %% 3iv.6. Compute the element stiffness matrix at the Gauss point
                    KeOnGP = computeElStiffMtxKirchhoffLoveShellLinear5DOF ...
