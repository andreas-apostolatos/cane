function [tanStiffMtx, resVct, BSplinePatch, propCoupling, minElArea] = ...
    computeTangentStiffMtxIGAKirchhoffLoveShellNLinear5DOF ...
...
% nonlinear analysis case to the isogeometrc Kirchhoff-Love shell using
...
%                                        a 3D array (5 DOFs per control point)
...
% Number of degrees of freedom for the whole structure (5 DOFs per control point)
numDOFs = 5*numCPs_xi*numCPs_eta;

% Number of degrees of freedom for the element (5 DOFs per control point)
numDOFsEl = 5*(p + 1)*(q + 1);
...
for j = q + 1:numKnots_eta - q - 1
    for i = p + 1:numKnots_xi - p - 1
        ...
        if (Xi(i + 1) ~= Xi(i) && Eta(j + 1) ~= Eta(j))
            %% 3i. Create an element freedom table (5 DOFs per control point)
            ...
            % relation global-local dof (5 DOFs per control point)
            for cpj = j - q:j
                for cpi = i - p:i
                    EFT(k) = DOFNumbering(cpi, cpj, 1);
                    EFT(k + 1) = DOFNumbering(cpi, cpj, 2);
                    EFT(k + 2) = DOFNumbering(cpi, cpj, 3);
                    EFT(k + 3) = DOFNumbering(cpi, cpj, 4);
                    EFT(k + 4) = DOFNumbering(cpi, cpj, 5);
                    k = k + 5;
                end
            end
            ...
            for cEta = 1:numGPeta
                for cXi = 1:numGPxi
                    ...
                    %% 3iv.8. Compute element tangent stiffness matrix and residual internal load vector
                    [KTElOnGP, FIElOnGP] = computeIGAElTangentStiffMtxKirchhoffLoveShellNLinear5DOF ...
