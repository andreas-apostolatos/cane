...
% HSDT shell formulation with 5 DOF per control point and the minimum 
% element area in the mesh for performing a refinement study.
%
...
%                                  (must include shear correction factor)
%                  .DOFNumbering : The numbering of the DOFs within the
%                                  patch itself (5 DOF per control point)
...
%                outMsg : Enables outputting information onto the cell when
%                         it is chosen as 'outputEnabled'
%
%                Output :
%                     K : master stiffness matrix
%                     F : The externally applied load vector
%          BSplinePatch : Updated BSplinePatch structure
...
% 0. Read input
%
% 1. Compute the material matrices (membrane, bending, shear)
%
% 2. Choose an integration rule
%
% 3. loops over elements
% ->
%    3i. Compute the determinant of the Jacobian transformation
%
%   3ii. Create the Element Freedom Table (5 DOF per CP)
%
%  3iii. Initialize the element area
%       
%   3iv. Loop over all the Gauss Points
%   ->
%        3iv.1. Compute the NURBS coordinates at the Gauss Point
%
%        3iv.2. Compute the NURBS basis functions and derivatives
%
%        3iv.3. Compute the covariant base vectors and derivatives
%
%        3iv.4. Compute the surface normal
%
%        3iv.5. Compute the area element
%
%        3iv.6. Compute the HSDT element stiffness matrix
%
%        3iv.7. Compute the element area and add contribution
%
%        3iv.8. Add the contribution to global stiffness matrix
%   <-
%    3v. Find the minimum element area in the mesh
% <-
% 
% 4. Compute the externally applied load vector
%
% 5. Check output
%
%% Function main body
...
% Local number of DOFs (5 DOF per control point for HSDT)
numDOFsEl = 5*(p + 1)*(q + 1);

% Total number of DOFs (5 DOF per control point)
...
% Compute the shear material matrix
% G = E/(2*(1+nu)) - shear modulus
G = parameters.E/(2*(1 + parameters.nue));
Ds = G*parameters.t*[1 0; 0 1];

% Shear correction factor (default 5/6 for shells)
if isfield(parameters, 'shearCorrection')
    shearCorrection = parameters.shearCorrection;
else
    shearCorrection = 5/6;  % Default Reissner-Mindlin value
end
...
for j = q + 1:numKnots_eta - q - 1
    for i = p + 1:numKnots_xi - p - 1
        if Xi(i + 1) ~= Xi(i) && Eta(j + 1) ~= Eta(j)
            ...
            %% 3ii. Create the Element Freedom Table (5 DOF per control point)
            ...
            % Relation global-local dof (5 DOF per control point)
            for cpj = j - q:j
                for cpi = i - p:i
            ...
            for iGP_eta = 1:numGP_eta
                for iGP_xi = 1:numGP_xi
                    ...
                    %% 3iv.6. Compute the HSDT element stiffness matrix
                    KeOnGP = computeElStiffMtxHSDTShellLinear ...
                        (p, q, dR, [dG1(:, 1) dG2(:, 1)], [dG1(:, 2) dG2(:, 2) ...
                        dG1(:, 3)], G3Tilde, Dm, Db, Ds, shearCorrection);
