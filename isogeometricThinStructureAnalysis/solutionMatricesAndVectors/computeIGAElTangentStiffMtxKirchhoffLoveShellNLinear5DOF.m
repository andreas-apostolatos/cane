function [KTel, FIEl] = ...
    computeIGAElTangentStiffMtxKirchhoffLoveShellNLinear5DOF ...
...
% vector at the element level for 5 DOFs per control point.
% 
%        Input : 
...
% Number of DoFs at the element level (5 DOFs per control point)
numDOFsEl = (p + 1)*(q + 1)*5;

% Initialize element tangent stiffness matrix
Kem = zeros(numDOFsEl, numDOFsEl);
Keb = zeros(numDOFsEl, numDOFsEl);

% Initialize element internal residual load vector
FInternalElementMembrane = zeros(numDOFsEl, 1);
FInternalElementBending = zeros(numDOFsEl, 1);

% Initialize auxuiliary arrays
dg = zeros(3, 2, numDOFsEl);
dg3Tilde = zeros(3, numDOFsEl);
g3Tildedg3Tilde = zeros(numDOFsEl);
g3dg3lg3_3 = zeros(numDOFsEl);
dn = zeros(3, numDOFsEl);

% Initialize arrays on the first variation of the strain and the curvature
% w.r.t. the DoFs
dECartesian = zeros(3, numDOFsEl);
dKCartesian = zeros(3, numDOFsEl);
...
%% 2. Compute nonlinear strain and curvature in the Cartesian coordinate system
...
for r = 1:numDOFsEl
    %% 3i. Get local node number kr and dof direction dirr (5 DOFs per control point)
    kr = ceil(r/5);
    dirr = r-5*(kr-1);
    
    % Only the first 3 DOFs (displacements) contribute to strain and curvature
    if dirr <= 3
        dg(dirr, 1, r) = dR(kr, 2);
        dg(dirr, 2, r) = dR(kr, 4);
        ...
        dg3Tilde(1, r) = dg(2, 1, r)*aCovariant(3, 2) - dg(3, 1, r)*aCovariant(2, 2) + ...
            aCovariant(2, 1)*dg(3, 2, r) - aCovariant(3, 1)*dg(2, 2, r);
        dg3Tilde(2, r) = dg(3, 1, r)*aCovariant(1, 2) - dg(1, 1, r)*aCovariant(3, 2) + ...
            aCovariant(3, 1)*dg(1, 2, r) - aCovariant(1, 1)*dg(3, 2, r);
        dg3Tilde(3, r) = dg(1, 1, r)*aCovariant(2, 2) - dg(2, 1, r)*aCovariant(1, 2) + ...
            aCovariant(1, 1)*dg(2, 2, r) - aCovariant(2, 1)*dg(1, 2, r);
        
        % Compute product g3Tilde*dg3Tilde
        g3Tildedg3Tilde(r) = a3Tilde(1)*dg3Tilde(1, r) + a3Tilde(2)*dg3Tilde(2, r) + ...
            a3Tilde(3)*dg3Tilde(3, r);
        ...
        dKCurvilinear(1, 1) = -(dR(kr, 3)*g3(dirr) + daCovariant(1, 1)*dn(1, r) + ...
            daCovariant(2, 1)*dn(2, r) + daCovariant(3, 1)*dn(3, r));
        dKCurvilinear(2, 1) = -(dR(kr, 6)*g3(dirr) + daCovariant(1, 2)*dn(1, r) + ...
            daCovariant(2, 2)*dn(2, r) + daCovariant(3, 2)*dn(3, r));
        dKCurvilinear(3, 1) = -(dR(kr, 5)*g3(dirr) + daCovariant(1, 3)*dn(1, r) + ...
            daCovariant(2, 3)*dn(2, r) + daCovariant(3, 3)*dn(3, r));
        
        % Compute curvature variation on the Cartesian system
        dKCartesian(:, r) = TFromContraToLC*dKCurvilinear;
    else
        % For DOFs 4 and 5 (rotations), set variations to zero for displacement-based formulation
        dECartesian(:, r) = zeros(3, 1);
        dKCartesian(:, r) = zeros(3, 1);
    end
end
...
% w.r.t. the DoFs
ddECartesian = zeros(3, numDOFsEl, numDOFsEl);
ddKCartesian = zeros(3, numDOFsEl, numDOFsEl);

% Compute second variation of strain and curvature w.r.t. dofs ur and us

% Loop over r DoFs
for r = 1:numDOFsEl
    %% 4i. Get local node number kr and dof direction dirr for DoF ur (5 DOFs per control point)
    kr = ceil(r/5);
    dirr = r - 5*(kr - 1);
    
    % Only consider displacement DOFs for strain and curvature
    if dirr <= 3
        % Loop over s DoFs for each r DoF
        for s = 1:r
            %% 4i.1. Get local node number ks and dof direction dirs for DoF us (5 DOFs per control point)
            ks = ceil(s/5);
            dirs = s - 5*(ks - 1);
            ...
            if dirs <= 3
                ...
                ddn = ddg3/norm(a3Tilde) - g3dg3lg3_3(s)*dg3Tilde(:, r) - ...
                    g3dg3lg3_3(r)*dg3Tilde(:, s) + C*a3Tilde + D*a3Tilde;
                
                % On the second variation of the curvature in the Curvilinear
                % coordinate system
                ddKCurvilinear(1, 1) = -(dR(kr, 3)*dn(dirr, s) + dR(ks, 3)*dn(dirs, r) + ...
                              daCovariant(1,1)*ddn(1) + daCovariant(2,1)*ddn(2) + ...
                              daCovariant(3,1)*ddn(3));
                ddKCurvilinear(2, 1) = -(dR(kr, 6)*dn(dirr, s) + dR(ks, 6)*dn(dirs, r) + ...
                              daCovariant(1, 2)*ddn(1) + daCovariant(2, 2)*ddn(2) + ...
                              daCovariant(3, 2)*ddn(3));
                ddKCurvilinear(3, 1) = -(dR(kr, 5)*dn(dirr, s) + dR(ks, 5)*dn(dirs,r) + ...
                              daCovariant(1, 3)*ddn(1) + daCovariant(2, 3)*ddn(2) + ...
                              daCovariant(3, 3)*ddn(3));
