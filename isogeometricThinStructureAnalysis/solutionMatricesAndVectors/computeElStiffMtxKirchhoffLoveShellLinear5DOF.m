function Ke = computeElStiffMtxKirchhoffLoveShellLinear5DOF ...
...
% The number of DOFs (5 DOFs per control point)
numCPsEl = (p + 1)*(q + 1);
...
for iDOFs = 1:numDOFsEl
    ...
    % Only first 3 DOFs contribute to membrane and bending (translational DOFs)
    if dir <= 3
        dg = zeros(3, 2);
        dg(dir, 1) = dR(k, 2);
        dg(dir, 2) = dR(k, 4);
    ...
    % DOFs 4 and 5 (rotational) don't contribute to membrane and bending in KL theory
end
