function index = plot_postprocIGAKirchhoffLoveShellMultipatchesNLinear...
...
% geometry. The visualization assumes a nonlinear strain measure and
% accounts for an arbitray number of Kirchhoff-Love shell multipatches of
% the coupled system.
%
%          Input :
...
if strcmp(outMsg,'outputEnabled')
    ...
    fprintf('nonlinear isogeometric multi-patch Kirchhoff-Love shell has been initiated\n\n');
    ...
    elseif strcmp(graph.resultant,'rotationExplicit')
        if strcmp(graph.component,'1')
            fprintf('Rotation component (explicit computation) omega^1\n');
        elseif strcmp(graph.component,'2')
            fprintf('Rotation component (explicit computation) omega^2\n');
        end
...
% Loop over all the patches in the coupled system
for counterPatches = 1:noPatches
    plot_postprocResultantsIGAKirchhoffLoveShellNLinear(BSplinePatches{counterPatches}.p,BSplinePatches{counterPatches}.q,BSplinePatches{counterPatches}.Xi,...
...
if strcmp(graph.resultant,'displacement')
    ...
    elseif strcmp(graph.component,'2norm')
        titleString = 'Displacement magnitude || d ||';
    end
elseif strcmp(graph.resultant,'strain')
    if strcmp(graph.component,'1')
...
elseif strcmp(graph.resultant,'rotation')
    ...
    elseif strcmp(graph.component,'2norm')
        titleString = 'Rotation magnitude \left\| \omega \right\|_2 ';
    end
elseif strcmp(graph.resultant,'rotationExplicit')
    if strcmp(graph.component,'1')
        titleString = 'Rotation component (explicit computation) \omega^{1}';
    elseif strcmp(graph.component,'2')
        titleString = 'Rotation component (explicit computation) \omega^{2}';
    end
elseif strcmp(graph.resultant,'curvature')
    if strcmp(graph.component,'1')
