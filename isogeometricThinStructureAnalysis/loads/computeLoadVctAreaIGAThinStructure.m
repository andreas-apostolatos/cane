...
if isFollower
    dRdXiMtx = zeros(noDOFsCP,noDOFsEl);
    dRdEtaMtx = zeros(noDOFsCP,noDOFsEl);
end
...
elseif strcmp(int.type,'user')
	noGPXi = int.xiNGPForLoad;
    noGPEta = int.etaNGPForLoad;
else
...
% On the xi-direction
xiSpanStart = findKnotSpan(xib(1),Xi,nxi);
xiSpanEnd = findKnotSpan(xib(2),Xi,nxi);

% On the eta-direction
etaSpanStart = findKnotSpan(etab(1),Eta,neta);
etaSpanEnd = findKnotSpan(etab(2),Eta,neta);
    ...
    for iXiSpan = xiSpanStart:xiSpanEnd
        ...
        if Xi(iXiSpan + 1)~=Xi(iXiSpan) && Eta(iEtaSpan + 1)~=Eta(iEtaSpan)
            ...
            for iGPEta = 1:noGPEta
                for iGPXi= 1:noGPXi
                    %% 3iii.1. Compute the coordinates, the map and the Gauss Point location and weight for the fixed parametric coordinate
                    xi = (Xi(iXiSpan + 1) + Xi(iXiSpan) + xiGP(iGPXi)*(Xi(iXiSpan + 1) - Xi(iXiSpan)))/2;
                    eta = (Eta(iEtaSpan+1) + Eta(iEtaSpan) + etaGP(iGPEta)*(Eta(iEtaSpan + 1)-Eta(iEtaSpan)))/2;
                    
                    %% 3iii.2. Compute the weight of the 2-dimensional quadrature
                    GW = xiGW(iGPXi)*etaGW(iGPEta);
                    
                    %% 3iii.3. Compute the IGA basis functions and their first derivatives
                    noDrv = 1;
                    dR = computeIGABasisFunctionsAndDerivativesForSurface...
                        (iXiSpan,p,xi,Xi,iEtaSpan,q,eta,Eta,CP,isNURBS,noDrv);
                    
                    %% 3iii.4. Compute the base vectors
                    noDrv = 0;
                    if isFollower
                        [a1,a2] = computeBaseVectorsAndDerivativesForBSplineSurface...
                            (iXiSpan,p,iEtaSpan,q,CPd,noDrv,dR);
                    else
                        [A1,A2] = computeBaseVectorsAndDerivativesForBSplineSurface...
                            (iXiSpan,p,iEtaSpan,q,CP,noDrv,dR);
                    end
