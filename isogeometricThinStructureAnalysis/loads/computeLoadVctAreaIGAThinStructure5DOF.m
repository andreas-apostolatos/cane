function [F, tanMtx] = computeLoadVctAreaIGAThinStructure5DOF ...
...
if strcmp(outMsg,'outputEnabled')
    ...
    if isvector(FOutdated)
        fprintf('Update of the load vector corresponding to area (5 DOF)\n');
    else
        fprintf('Computation of the load vector corresponding to area (5 DOF)\n');
    end
    fprintf('loads over a B-Spline surface\n');
    fprintf('_______________________________________________________\n\n');
    tic;
end
...
% read Control Point coordintes
CP = BSplinePatch.CP;

% read whether the surface is a NURBS or a B-Spline
...
% Number of Control Points affecting an element
noCPsEl = (p + 1)*(q + 1);
...
% Deduce load integration scheme
if isstruct(propInt)
        ...
        if Xi(iXiSpan + 1) ~= Xi(iXiSpan) && Eta(iEtaSpan + 1) ~= Eta(iEtaSpan)
            ...
            for iGPEta = 1:numGP_eta
                for iGPXi= 1:numGP_xi
                    ...
                    %% 3iii.7. Compute the matrices containing the basis functions and their parametric derivatives
                    for iCPs = 1:noCPsEl
                        % Only first 3 DOFs (translational) get the load contribution
                        RMtx(1, 5*iCPs - 4) = dR(iCPs, 1);  % x-displacement
                        RMtx(2, 5*iCPs - 3) = dR(iCPs, 1);  % y-displacement  
                        RMtx(3, 5*iCPs - 2) = dR(iCPs, 1);  % z-displacement
                        % DOFs 4 and 5 (rotational) don't get load contribution in this implementation
                        
                        if isFollower && strcmp(direction, 'normal')
                            dRdXiMtx(1, 5*iCPs - 4) = dR(iCPs, 2);
                            dRdXiMtx(2, 5*iCPs - 3) = dR(iCPs, 2);
                            dRdXiMtx(3, 5*iCPs - 2) = dR(iCPs, 2);
                            dRdEtaMtx(1, 5*iCPs - 4) = dR(iCPs, 3);
                            dRdEtaMtx(2, 5*iCPs - 3) = dR(iCPs, 3);
                            dRdEtaMtx(3, 5*iCPs - 2) = dR(iCPs, 3);
                        end
                    end
