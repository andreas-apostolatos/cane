...
fileName = 'FoFiDDMFourPointSailFine';
...
save(['data_ModalAnalysis' method 'Fine']);
return;

%% Load a reference solution for the modal analysis
modalAnalysisSP = importdata('./data_ModalAnalysisPool/data_modalAnalysisfourPointSailFoFiFourPointSailp3q3Xi50Eta50.mat');
...
%% Update the geometry accrording to the computed modal shape and write an carat++ input file

% Update the geometry accrording to the computed modal shape
noEigen = 2;
scalingFct = -20;
dHatMA = eigenmodeShapes(EFTPatchesWTLM,noEigen);
for iPatches = 1:noPatches
    CPd = computeDisplacedControlPointsForIGAKirchhoffLoveShell...
        (BSplinePatchesEq{iPatches}.CP,scalingFct*dHatMA(BSplinePatches{iPatches}.EFTPatches));
    BSplinePatchesEq{iPatches}.CP = CPd;
end
color = [217 218 219]/255;
graph.index = plot_referenceConfigurationIGAThinStructureMultipatches...
    (BSplinePatchesEq,connections,color,graph,'outputEnabled');
az = 260;
el = 30;
view(az,el);
camlight(30,70);
...
% write an carat++ input file
strongDBC = {};
pathToOutput = '../../../inputCarat/isogeometricMembraneAnalysis/';
writeOutMultipatchBSplineSurface4Carat...
    (BSplinePatchesEq,strongDBC,connections,pathToOutput,caseName);
clear BSplinePatchesEq;
