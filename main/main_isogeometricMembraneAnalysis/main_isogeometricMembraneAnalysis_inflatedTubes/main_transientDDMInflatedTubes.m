...
% 
% Task : Transient simulation over a multipatch roof made of inflated tubes
%
% Date : 21.02.2017
%
%% Preamble
...
% Case name
caseName = 'transientDDMInflatedTubes';
...
method = 'AugmentedLagrangeMultipliers';
...
% Mesh size
meshSize = 'coarse'; % 'dummy', 'coarse', 'fine'
...
logA0y = 4.0;
A0 = 10.0; % 1.0
A0y = A0*10^logA0y*1e-6;
logA0x = 2.0;
A0x = A0*10^logA0x*1e-6;
logA0z = 1.0;
A0z = A0*10^logA0z*1e-6;
...
if strcmp(method,'LagrangeMultipliers') || strcmp(method,'AugmentedLagrangeMultipliers')
    ...
    weakDBC.imposedMotion = {@(x,y,z,t) [A0x*sin(omegax*t); A0y*sin(omegay*t); A0z*sin(omegaz*t)]};
    ...
    if strcmp(method,'LagrangeMultipliers') || strcmp(method,'AugmentedLagrangeMultipliers')
        ...
        for iCnd = 1:length(xiExtension)
            for iDOFs = 1:length(etaExtension)
                for iCoord = 1:3
                    inhomDOFs = findDofs3D(inhomDOFs,xiExtension,etaExtension,iCoord,CP);
                end
            end
        end
        for iInhomDBC = 1:length(inhomDOFs)/3
            valuesInhomDOFs{3*iInhomDBC-2} = @(t) A0x*sin(omegax*t);
...
% BSplinePatchesMod = {BSplinePatches{1} BSplinePatches{2} BSplinePatches{3} BSplinePatches{4} ...
%     BSplinePatches{5} BSplinePatches{6} BSplinePatches{7} BSplinePatches{8}};
% graph.index = plot_referenceConfigIGAMortarMapping...
%     (BSplinePatchesMod,msh,labelsEnabled,color,graph);
...
elseif strcmp(method,'Mortar')
    ...
    for iPatches = 1:noPatches
        ...
        for dir = 1:3
            BSplinePatches{iPatches}.inhomDOFs = findDofs3D...
                (BSplinePatches{iPatches}.inhomDOFs,xisup,etasup,dir,BSplinePatches{iPatches}.CP);
        end
        BSplinePatches{iPatches}.valuesInhomDOFs = cell({});
        for iInhomDBC = 1:length(BSplinePatches{iPatches}.inhomDOFs)/3
            BSplinePatches{iPatches}.valuesInhomDOFs{3*iInhomDBC-2} = @(t)A0x*sin(omegax*t);
            BSplinePatches{iPatches}.valuesInhomDOFs{3*iInhomDBC-1} = @(t)A0y*sin(omegay*t);
            BSplinePatches{iPatches}.valuesInhomDOFs{3*iInhomDBC} = @(t)A0z*sin(omegaz*t);
        end
...
if strcmp(meshSize,'coarse')
    timeStepScaling = 800; % 12
elseif strcmp(meshSize,'fine')
    timeStepScaling = 100; % 12
end
...
%     functionHandle = weakDBC1.imposedMotion{1};
%     vct = functionHandle(0,0,0,timeInstance);
%     fprintf('   TIME = 	%d		VAL = 	%d\n',timeInstance,vct(3,1));
...
[dHatHistory,resHistory,BSplinePatches,propCoupling,~] = ...
    ...
    propOutput,pathToOutput,[caseName '_' meshSize '_' method],...
...
% scaling = 20.0;
% t = 2.65;
noTimeStep = int64((t - propStrDynamics.TStart)/propStrDynamics.dt + 1);
graph.index = plot_postprocIGAMembraneMultipatchesNLinear...
...
nameFileMeshInfo = '../../../preComputedData/isogeometricMembraneAnalysis/inflatedTubes/referenceFEMSolution_TransientAnalysis/output1_1_recorderClient_myMesh1.msh';
nameFileMeshResults = '../../../preComputedData/isogeometricMembraneAnalysis/inflatedTubes/referenceFEMSolution_TransientAnalysis/output1_1_recorderClient_myMesh1.res';
fileMeshInfo = fileread(nameFileMeshInfo);
fileMeshResults = fileread(nameFileMeshResults);

% Get the nodes of the mesh
stringSeparator = 'MESH "mesh_quads" dimension 3 Elemtype Quadrilateral Nnode 4';
...
%% Re-arrange the displacement field
% scaling = 20.0;
scaling = 1.0;
...
resultant = 'stress'; % 'displacement', 'strain', 'stress'
component = '2Principal'; % 'x', 'y', 'z', '1Principal', '2Principal', '2norm'
