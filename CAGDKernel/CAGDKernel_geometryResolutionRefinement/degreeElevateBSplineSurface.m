...
if tp > 0
    ...
    for i = 1:fix(pr/2)   % bezalfs are symmetric to row pr/2
        ...
        for j = max(0, i - tp):mpi
            bezalfs(i + 1, j + 1) = inv*getBinomialCoefficients(p, j)* ...
                getBinomialCoefficients(tp, i - j);
                ...
                while j - i > tr
                    ...
                    if j >= lbz
                        if j - tr <= kind - pr + oldr
                            ...
                            ebpts(kj + 1, :, :) = gamma*ebpts(kj + 1, :, :) + ...
                                (1.0 - gamma)*ebpts(kj + 2, :, :);
                        else
                            ebpts(kj + 1, :, :) = beta*ebpts(kj + 1, :, :) + ...
                                (1.0 - beta)*ebpts(kj + 2, :, :);
                        end
                    end
                    i = i + 1;
...
if tq > 0
    ...
    for i = 1:fix(qr/2)
        ...
        for j = max(0, i - tq):mpi
            bezalfs(i + 1, j + 1) = inv*getBinomialCoefficients(q, j)* ...
                getBinomialCoefficients(tq, i - j);
