function [inactive_nodes] = detectInactiveNodes(DOF, candidateNodes, dexp, segments, gap )
...
%     candidateNodes : List of indices of the nodes which shall be
...
%
inactive_nodes=[];
for i=1:length(candidateNodes)%loop over every node
    % computation is valid for 2D analysis with 2 displacement DoFs per node
    tmp_normal=dot(dexp(2*candidateNodes(i)-1:1:2*candidateNodes(i)),segments.normals(1,:));
    tmp_parallel=dot(dexp(2*candidateNodes(i)-1:1:2*candidateNodes(i)),segments.directors(1,:));
